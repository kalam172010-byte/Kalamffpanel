...
function patchDepositAdminSync(filePath) {
  ...
  // AND add automatic predictive QR preloading so candidate QR codes are in browser cache before checkout
  ...
  const cleanOpenEffect = 'Qs.useEffect(()=>{if(n){d("AMOUNT"),W(null),P(null),ue(null);try{if(s&&s.customQrUrl){const cImg=new Image();cImg.src=s.customQrUrl}const mU=(V&&(V.upiId||V.merchantUpi))||(s&&(s.upiManualId||s.upiId||s.merchantUpi))||"kalamffpanel@fampay";const mN=(V&&V.merchantName)||(s&&(s.upiMerchantName||s.shopName))||"KALAM FF PANEL";[Number(i)||((s==null?void 0:s.minDeposit)||10),50,100,200,500].forEach(amt=>{if(amt>0){const pIt="upi://pay?pa="+encodeURIComponent(mU)+"&pn="+encodeURIComponent(mN)+"&am="+amt+"&cu=INR";const pQr="https://api.qrserver.com/v1/create-qr-code/?size=300x300&margin=8&data="+encodeURIComponent(pIt);const pI=new Image();pI.src=pQr}})}catch{}}},[n]);';
  if (code.includes(oldOpenEffect)) {
    code = code.replace(oldOpenEffect, cleanOpenEffect);
    changed = true;
    console.log('[3] Fixed modal open effect in LQ: starts at clean AMOUNT screen with dynamic QR preloading');
  }
  ...
  // CRITICAL: Preload candidate QR while request is in flight and immediately preload realQr when response arrives!
  ...
  if (jStart !== -1 && jEnd !== -1) {
    const newJ = 'const J=()=>{const currAmt=Number(i)||((s==null?void 0:s.minDeposit)||10);if(!currAmt||currAmt<((s==null?void 0:s.minDeposit)||1)){P("Please enter a valid deposit amount (minimum ₹"+((s==null?void 0:s.minDeposit)||1)+").");return;}l(currAmt);P(null);ue(null);pe(300);d("GENERATING");const mUpi=(V&&(V.upiId||V.merchantUpi))||(s&&(s.upiManualId||s.upiId||s.merchantUpi))||"kalamffpanel@fampay";const mName=(V&&V.merchantName)||(s&&(s.upiMerchantName||s.shopName))||"KALAM FF PANEL";const gwType=((V==null?void 0:V.baseUrl)||"").includes("famgateway")?"famgateway":((V==null?void 0:V.baseUrl)||"").includes("aditya")?"adityahost":((V==null?void 0:V.baseUrl)||"").includes("zap")?"zapupi":"freepanel";try{const predIntent="upi://pay?pa="+encodeURIComponent(mUpi)+"&pn="+encodeURIComponent(mName)+"&am="+currAmt+"&cu=INR";const predQr=(s!=null&&s.customQrUrl)?s.customQrUrl:("https://api.qrserver.com/v1/create-qr-code/?size=300x300&margin=8&data="+encodeURIComponent(predIntent));const preImg=new Image();preImg.src=predQr;if(typeof document!=="undefined"){const lk=document.createElement("link");lk.rel="preload";lk.as="image";lk.href=predQr;document.head.appendChild(lk)}}catch{}Gr("/api/create-order",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({amount:currAmt,redirect_url:window.location.origin+"/success",apiKey:V==null?void 0:V.apiKey,gatewayUrl:(V==null?void 0:V.baseUrl)||(V==null?void 0:V.gatewayUrl),merchantUpi:mUpi,gateway:gwType})},20000).then(st=>{if(st!=null&&st.data&&st.data.order){const gOrd=st.data.order;const realOrderId=gOrd.orderId;const realPayee=gOrd.payeeUpi||gOrd.upi_id||mUpi;const realName=gOrd.merchantName||mName;const realIntent=gOrd.upiIntent||`upi://pay?pa=${encodeURIComponent(realPayee)}&pn=${encodeURIComponent(realName)}&tr=${encodeURIComponent(realOrderId)}&tn=${encodeURIComponent("Payment for Order "+realOrderId)}&am=${currAmt}&cu=INR`;const realQr=(s!=null&&s.customQrUrl)?s.customQrUrl:(gOrd.qrUrl||("https://api.qrserver.com/v1/create-qr-code/?size=300x300&margin=8&data="+encodeURIComponent(realIntent)));try{const qrPreload=new Image();qrPreload.decoding="sync";qrPreload.src=realQr;if(typeof document!=="undefined"){const lk=document.createElement("link");lk.rel="preload";lk.as="image";lk.href=realQr;document.head.appendChild(lk)}}catch{}W({...gOrd,orderId:realOrderId,amountInPaise:currAmt*100,amountInRupees:currAmt,paymentUrl:gOrd.paymentUrl||gOrd.checkoutUrl||realIntent,checkoutUrl:gOrd.checkoutUrl||gOrd.paymentUrl,upiIntent:realIntent,payeeUpi:realPayee,merchantName:realName,qrUrl:realQr});d("QR");}else{const errMsg=(st&&st.data&&st.data.error)||(st&&st.error)||"Failed to generate payment order from gateway. Check API key in Admin.";P(errMsg);d("AMOUNT");}}).catch(err=>{P((err&&err.message)||"Connection to payment gateway failed. Please check internet or gateway API key in Admin.");d("AMOUNT");});};  ';
    code = code.slice(0, jStart) + newJ + code.slice(jEnd);
    changed = true;
    console.log('[4] Fixed J() in LQ to use QR code preloading, 20s timeout and strict gateway order ID validation');
  }

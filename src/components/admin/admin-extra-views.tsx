...
import { AuthUser, StoreSettings, ResellerUser, Product, ApiConfig, PlanPricing, PurchasedKey, TransactionRecord, DiscountCoupon } from '../../types';
import { deduplicateUsers } from '../../lib/firestore-service';
import { exportSoldKeysToPdf, exportAccountingTransactionsToPdf, exportAccountingTransactionsToCsv } from '../../lib/pdf-export';
import { getStoredCoupons, saveStoredCoupons } from '../../lib/coupon-service';
...
export const AdminSoldKeysView: React.FC<AdminSoldKeysViewProps> = ({
  ...
  // Export official accounting transaction history to PDF
  const handleDownloadAccountingPdf = () => {
    try {
      setIsExportingAccounting(true);
      const res = exportAccountingTransactionsToPdf({
        soldKeys: filteredKeys,
        transactions,
        storeSettings,
        filterLabel:
          filterProduct !== 'ALL'
            ? `Product Filter: ${filterProduct}`
            : 'Complete Sales & Wallet Ledger',
        adminName: currentUser?.name || 'Administrator',
      });
      setExportFeedback(`Exported ${res.count} accounting transactions to ${res.filename}`);
  ...
  // Export official accounting transaction history to CSV / Excel
  const handleDownloadAccountingCsv = () => {
    try {
      setIsExportingCsv(true);
      const res = exportAccountingTransactionsToCsv({
        soldKeys: filteredKeys,
        transactions,
        storeSettings,
        filterLabel:
      ...
      setExportFeedback(`Exported ${res.count} accounting records to CSV (${res.filename})`);
      setTimeout(() => setExportFeedback(null), 4500);
    } catch (err: any) {
      setExportFeedback(`Export error: ${err.message}`);
      setTimeout(() => setExportFeedback(null), 4000);
  ...
  // Export license keys & serial codes to PDF
    ...
    <div className="space-y-4" id="admin-sold-keys-view">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* Primary Download PDF Button for Accounting Purposes */}
          <motion.button
            id="download-pdf-btn"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.97 }}
            ...
            className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-[0_0_15px_rgba(16,185,129,0.35)] disabled:opacity-50 cursor-pointer"
            title="Download official transaction history & financial ledger for accounting and tax purposes"
          >
          ...
          {/* Download CSV / Excel Spreadsheet Button */}
          <motion.button
            id="download-csv-btn"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.97 }}
            ...
            className="px-3.5 py-2 rounded-xl bg-teal-500/20 hover:bg-teal-500/30 border border-teal-500/40 text-teal-300 font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
            title="Download ledger spreadsheet for Microsoft Excel and Google Sheets"
          >
            {isExportingCsv ? (
              <RefreshCw className="w-3.5 h-3.5 animate-spin" />
            ) : (
          ...
          {/* Secondary Button: Keys & Serial List */}
          {soldKeys.length > 0 && (
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.97 }}
              ...
              className="px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300 hover:text-white font-medium text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
              title="Download detailed license keys serial list"
            >
              {isExporting ? (
                <RefreshCw className="w-3.5 h-3.5 animate-spin" />
              ) : (
              ...
              <span>Keys Only ({filteredKeys.length})</span>
            </motion.button>
          )}
        </div>
      </div>
      ...
      <GlassCard glow="green" className="p-3.5 bg-[#161622]/95 border-emerald-500/20 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
        <div className="space-y-0.5">
          <span className="text-[10px] text-gray-400 font-semibold block">Gross Sales Revenue</span>
          <span className="text-sm font-black font-mono text-emerald-400">
            ₹{totalRevenue.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
          </span>
        </div>
        <div className="space-y-0.5">
          <span className="text-[10px] text-gray-400 font-semibold block">Total Invoices / Orders</span>
          <span className="text-sm font-bold font-mono text-white">
            {filteredKeys.length} {filteredKeys.length === 1 ? 'Sale' : 'Sales'}
          </span>
        </div>
        <div className="space-y-0.5">
          <span className="text-[10px] text-gray-400 font-semibold block">Wallet Transactions</span>
          <span className="text-sm font-bold font-mono text-cyan-400">
            {transactions.length} Records
          </span>
        </div>
        <div className="space-y-0.5">
          <span className="text-[10px] text-gray-400 font-semibold block">Accounting Export</span>
          <span className="text-[11px] font-semibold text-emerald-300 flex items-center gap-1">
            <CheckCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            Audit Statement Ready
          </span>
        </div>
      </GlassCard>
      ...
      {soldKeys.length === 0 ? (
        <GlassCard className="p-8 text-center space-y-3 bg-[#161622]/90 border-white/10">
          ...
          <h3 className="text-sm font-bold text-white">No Sold Keys in System Yet</h3>
          <p className="text-xs text-gray-400 max-w-sm mx-auto">
            When users or resellers purchase keys, their live delivery records and transaction history will stream here in real time.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
            {/* Download PDF button works even on empty state to provide zero-balance official statement */}
            <button
              type="button"
              onClick={handleDownloadAccountingPdf}
              disabled={isExportingAccounting}
              className="px-3.5 py-2 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 text-emerald-300 text-xs font-bold transition-all cursor-pointer inline-flex items-center gap-1.5"
            >
              <Download className="w-3.5 h-3.5" />
            ...
            <button
              ...
              onClick={handleDownloadAccountingCsv}
              disabled={isExportingCsv}
              className="px-3.5 py-2 rounded-xl bg-teal-500/20 hover:bg-teal-500/30 border border-teal-500/40 text-teal-300 text-xs font-bold transition-all cursor-pointer inline-flex items-center gap-1.5"
            >
            ...
            {onSeedSampleKeys && (
              <button
                type="button"
                onClick={() => {
                  const sampleKeys: PurchasedKey[] = [
                    {
                      id: `key-sample-${Date.now()}-1`,
                      productName: 'FF MAX VIP AIMBOT (VIP Hack)',
                      planName: '30 Days License',
                      keyCode: 'KLM-MAX-9921-VIP-8819',
                      purchaseDate: new Date().toLocaleString(),
                      expiryDate: 'In 30 days',
                      status: 'ACTIVE',
                      price: 850,
                      invoiceNumber: `INV-SAMPLE-101`,
                      orderId: 'ORD-FFM-8910',
                      deviceType: 'Android (No Root)',
                    },
                    {
                      id: `key-sample-${Date.now()}-2`,
                      productName: 'FF ESP RADAR PRO (VIP Hack)',
                      planName: '7 Days License',
                      keyCode: 'KLM-ESP-7712-RAD-3391',
                      purchaseDate: new Date(Date.now() - 3600000 * 5).toLocaleString(),
                      expiryDate: 'In 7 days',
                      status: 'ACTIVE',
                      price: 350,
                      invoiceNumber: `INV-SAMPLE-102`,
                      orderId: 'ORD-ESP-4421',
                      deviceType: 'Android / iOS',
                    },
                    {
                      id: `key-sample-${Date.now()}-3`,
                      productName: 'BRUTAL HEADSHOT BR (VIP Panel)',
                      planName: '1 Day License',
                      keyCode: 'KLM-BRU-1102-HSH-9924',
                      purchaseDate: new Date(Date.now() - 3600000 * 24).toLocaleString(),
                      expiryDate: 'Expired',
                      status: 'EXPIRED',
                      price: 120,
                      invoiceNumber: `INV-SAMPLE-103`,
                      orderId: 'ORD-BRU-1120',
                      deviceType: 'PC / Emulator',
                    },
                  ];
                  onSeedSampleKeys(sampleKeys);
                  setExportFeedback('Loaded 3 sample transactions for accounting demonstration.');
                  setTimeout(() => setExportFeedback(null), 3000);
                }}
                className="px-3.5 py-2 rounded-xl bg-cyan-500/20 hover:bg-cyan-500/30 border border-cyan-500/40 text-cyan-300 text-xs font-bold transition-all cursor-pointer inline-flex items-center gap-1.5"
              >
                <Plus className="w-3.5 h-3.5" />
                <span>Load Sample Records For Accounting Demo</span>
              </button>
            )}
          </div>
        </GlassCard>
...
export const AdminCouponManagerView: React.FC = () => {
  ...
  const handleCreateCoupon = (e: React.FormEvent) => {
    ...
    if (coupons.some((c) => c.code.toUpperCase() === cleanCode)) {
      setFeedback(`Coupon code '${cleanCode}' already exists!`);
      setTimeout(() => setFeedback(null), 3000);
      return;
    }

    const pct = Number(discountPercent) || 0;
    const flat = Number(discountFlat) || undefined;
    const min = Number(minAmount) || undefined;
    ...
    const newCoupon: DiscountCoupon = {
      ...
      discountFlat: flat,
      minAmount: min,
      description: description || `${pct > 0 ? `${pct}% OFF` : `Flat ₹${flat} OFF`} Special Promo`,
      isActive: true,
      maxUses: 1000,
      usedCount: 0,
    };
    ...
    setDiscountFlat('');
    setMinAmount('');
    setFeedback(`🎉 Coupon '${cleanCode}' created and published live to store!`);
  ...
  return (
    <div className="space-y-5" id="admin-coupon-view">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-extrabold text-white flex items-center gap-2">
            <Ticket className="w-4 h-4 text-yellow-400" />
            <span>Store Promo & Coupon Manager ({coupons.length})</span>
          </h2>
          <span className="text-[11px] text-gray-400">
            Create and manage promotional discount coupons for user checkouts
          </span>
        </div>
      </div>
      ...
      {feedback && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="p-3 rounded-xl bg-yellow-500/15 border border-yellow-500/30 text-yellow-300 text-xs font-semibold flex items-center gap-2"
        >
      ...
      {/* Create Coupon Form */}
      <GlassCard glow="gold" className="p-4 bg-[#161622]/95 border-yellow-500/30 space-y-3.5 text-xs">
        ...
        <form onSubmit={handleCreateCoupon} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-gray-300 font-semibold mb-1">Coupon Code (Promo Code)</label>
              <input
                type="text"
                required
                value={couponCode}
                onChange={(e) => setCouponCode(e.target.value.toUpperCase())}
                placeholder="e.g. KALAM50 or VIP20"
                className="w-full px-3 py-2 rounded-xl bg-black/50 border border-white/10 focus:border-yellow-400 text-white font-mono uppercase text-xs"
              />
            </div>

            <div>
              <label className="block text-gray-300 font-semibold mb-1">Percentage Discount (%)</label>
              <input
                type="number"
                min="0"
                max="100"
                ...
                placeholder="e.g. 20"
                className="w-full px-3 py-2 rounded-xl bg-black/50 border border-white/10 focus:border-yellow-400 text-white font-bold text-xs"
              />
            </div>

            <div>
              <label className="block text-gray-300 font-semibold mb-1">Or Flat Discount (₹)</label>
              <input
                type="number"
                min="0"
                value={discountFlat}
                ...
                className="w-full px-3 py-2 rounded-xl bg-black/50 border border-white/10 focus:border-yellow-400 text-white text-xs font-mono"
              />
            </div>

            <div>
              <label className="block text-gray-300 font-semibold mb-1">Minimum Order Amount (₹)</label>
              <input
                type="number"
                min="0"
                value={minAmount}
          ...
          <div>
            <label className="block text-gray-300 font-semibold mb-1">Description / Promo Badge</label>
            <input
              type="text"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="e.g. Flash 20% discount on all Free Fire VIP keys"
              className="w-full px-3 py-2 rounded-xl bg-black/50 border border-white/10 focus:border-yellow-400 text-white text-xs"
            />
          </div>

          <button
            type="submit"
            className="w-full py-2.5 rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 hover:from-yellow-400 hover:to-amber-500 text-black font-extrabold text-xs shadow-lg cursor-pointer transition-all flex items-center justify-center gap-1.5"
          >
            <Plus className="w-3.5 h-3.5" />
            <span>Publish Discount Coupon</span>
          </button>
        </form>
      </GlassCard>
      ...
      <div className="space-y-2">
        <h3 className="text-xs font-bold text-gray-300 uppercase tracking-wider">
          Active Coupons in Store Pool ({coupons.length})
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {coupons.map((c) => (
            <GlassCard
              key={c.id}
              className={`p-3.5 bg-[#161622]/90 border text-xs space-y-2 ${
                c.isActive ? 'border-yellow-500/30' : 'border-white/10 opacity-60'
              }`}
            >
              <div className="flex items-start justify-between">
                <div>
                  <span className="font-mono font-black text-sm text-yellow-400 block tracking-wide">
                    {c.code}
                  </span>
                  <span className="text-[11px] text-gray-300 block">{c.description}</span>
                </div>

...
export const UserDashboard: React.FC<UserDashboardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="space-y-4"
    >
      {/* 0. PROMINENT STORE BRANDING & IDENTITY HERO (Customizable in Admin Store Settings) */}
      {storeSettings?.showHeroCard !== false && (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#161622] via-[#1a1329] to-[#0f172a] border border-[#8b5cf6]/30 p-4 shadow-[0_0_25px_rgba(139,92,246,0.15)]">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div
                className={`w-12 h-12 bg-gradient-to-tr from-[#00e5ff] via-[#8b5cf6] to-[#ff0080] p-[1.5px] shadow-[0_0_15px_rgba(0,229,255,0.4)] shrink-0 ${
                  storeSettings?.logoShape === 'circle'
                    ? 'rounded-full'
                    : storeSettings?.logoShape === 'square'
                    ? 'rounded-xl'
                    : 'rounded-2xl'
                }`}
              >
            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.2)]">
                ...
                <span>{storeSettings?.storeNoticeBadge || '100% Direct Credit'}</span>
              </span>
      ...
      {/* 4. QUICK ACTIONS GRID (3 cols, 2 rows - customizable in store settings) */}
      {storeSettings?.showQuickActions !== false && (
      ...
      {/* 5. LIVE STORE PRODUCTS PREVIEW (customizable in store settings) */}
      {storeSettings?.showCatalogPreview !== false && (

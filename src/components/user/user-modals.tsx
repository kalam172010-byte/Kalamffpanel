...
export const DepositModal: React.FC<DepositModalProps> = ({
  ...
  const handleInstantAutoCheck = async () => {
    ...
    try {
      ...
      if (data?.isPaid === true && data?.status === 'SUCCESS') {
        ...
        setAutoCheckStatus('Payment pending: If already paid, enter 12-digit UTR below for instant credit.');
      }
    } catch (e) {
      console.warn(e);
  ...
  const handleVerifyUtr = async () => {
    ...
    if (!clean) {
      setVerifyError('Please enter the 12-digit UPI UTR / Ref number from your payment app receipt.');
      return;
    }
    ...
    try {
      ...
      if (data?.isPaid === true && data?.status === 'SUCCESS') {
        ...
        setVerifyError(data?.message || 'Payment not verified yet by bank. If you completed payment, please wait a moment and retry.');
      }
    } catch (err: any) {
      setVerifyError('Unable to verify with payment gateway. Please ensure your UPI payment is completed and retry.');
    } finally {
  ...
  const handlePasteUtr = async () => {
    try {
      ...
      if (text) {
        // extract digits or alphanumeric code
        const cleaned = text.replace(/[^0-9A-Za-z]/g, '').trim();
        setUtrInput(cleaned || text.trim());
      }
    } catch (err) {
            ...
            <div className={step === 'QR' ? 'space-y-4' : 'space-y-4'}>
              ...
              {step === 'QR' && (
                <div className="space-y-4">
                  ...
                  {/* UTR / REFERENCE VERIFICATION SECTION */}
                  <div className="p-3.5 rounded-2xl bg-gradient-to-r from-[#130d29] to-[#1c113b] border border-[#8b5cf6]/30 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <CreditCard className="w-3.5 h-3.5 text-[#c084fc]" />
                        <span className="text-xs font-bold text-white">Enter 12-Digit UTR / UPI Ref</span>
                      </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-1.5">
                        <div className="relative flex-1">
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') handleVerifyUtr();
                            }}
                            className="w-full pl-3 pr-8 py-2 rounded-xl bg-black/60 border border-white/15 focus:border-[#c084fc] focus:outline-none text-xs font-mono font-bold text-white placeholder:text-gray-500 tracking-wider"
                          />
                          {utrInput && (
                            <button
                              onClick={() => setUtrInput('')}
                        ...
                        <button
                          ...
                          className="px-2.5 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-[11px] font-bold text-gray-200 border border-white/10 transition-colors flex items-center gap-1 shrink-0"
                      ...
                      {verifyError && (
                        <div className="p-2 rounded-lg bg-red-950/60 border border-red-500/40 text-[11px] text-red-300 flex items-start gap-1.5">
                      ...
                      <button
                        ...
                        className="w-full py-2.5 rounded-xl bg-gradient-to-r from-[#00e5ff] to-[#00b4d8] text-[#0a0a0f] font-extrabold text-xs shadow-[0_0_15px_rgba(0,229,255,0.35)] flex items-center justify-center gap-2 cursor-pointer transition-all disabled:opacity-50"
                      >
                        {isSubmittingUtr ? (
                          <>
                            <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            <span>Verifying Bank Receipt...</span>
                          </>
                        ) : (
                          <>
                            <Check className="w-3.5 h-3.5 stroke-[2.5]" />
                            <span>Submit UTR &amp; Confirm Deposit</span>
                          </>
                        )}
                      </button>
                    </div>
                  ...
                  <div className="p-3 rounded-2xl bg-[#090514] border border-white/10 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                    ...
                    <div className="pt-0.5">
                      <button
                        ...
                        className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-white/10 disabled:opacity-50 text-white font-bold text-[11px] flex items-center justify-center gap-1.5 cursor-pointer transition-all border border-white/10"
                      >
                        <RefreshCw className={`w-3 h-3 text-cyan-400 ${isManualChecking ? 'animate-spin' : ''}`} />
                        <span>{isManualChecking ? 'Detecting Payment Transfer...' : 'Auto-Check Bank Status'}</span>
                      </button>
                    </div>
                  </div>
